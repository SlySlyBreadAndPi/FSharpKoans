...
module ``19: Other list functions`` =
        ...
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec innerF xs =
                ...
                | a::rest -> match f a with
                             |true-> true
                             |false -> innerF rest
            innerF xs // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``02 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec innerF x (out1,out2)=
                match x with
                | [] -> out1,out2 
                | a::rest -> match f a with
                             |true-> innerF rest (out1@[a],out2)
                             |false -> innerF rest (out1,out2@[a])
            innerF xs ([],[])  // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
    ...
    let ``03 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec innerF x out=
                match x with
                | 0 -> out 
                | _ -> innerF (x-1) ((f (x-1))::out)
            innerF (n) [] // Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
    ...
    *)
    ...
    let ``07 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec innerF x i out=
                match x with
                | [] -> out 
                | a::rest -> innerF rest (i+1) (out@[f i a])
            innerF xs 0 [] // Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
