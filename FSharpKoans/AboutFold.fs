...
module ``17: Welcome to the functional fold`` =
    ...
    let ``01 A fold which sums a list`` () =
        let fold initialState xs =
            let rec innerF initial xs =
                match xs with
                | [] -> initial 
                | a::rest -> innerF (initial+a) rest
            innerF initialState xs // write a function to do what's described above
    ...
    let ``02 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec innerF initial xs =
                ...
                | [] -> initial 
                | a::rest -> innerF (initial*a) rest
            innerF initialState xs // write a function to multiply the elements of a list
    ...
    let ``03 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec innerF initial xs =
                match xs with
                | [] -> initial 
                | a::rest -> innerF (f initial a) rest
            innerF initialState xs  // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
